...
class SitusController extends AdminController {
	...
	public function editNasional()
	{
		...
		if (\Auth::user()->Region == 'Nasional') {
			...
			return \View::make('back.situs.edit_nasional', compact('data'));
		}
	...
	public function update()
	{
		...
		if (\Auth::user()->Region == 'Provinsi') {
			...
			$data->backlog =  \Input::get('backlog');
			$data->rtlh =  \Input::get('rtlh');
			$data->sejuta_rumah =  \Input::get('sejuta_rumah');
	...
	public function updateNasional()
	{
		...
		if (\Auth::user()->Region == 'Nasional') {
			...
			if($data->save()) {
				...
				return \Redirect::route('back-office.nasional.setting.edit')
					->with('message', 'Data berhasil diubah')
					->with('class', 'success');
			}
